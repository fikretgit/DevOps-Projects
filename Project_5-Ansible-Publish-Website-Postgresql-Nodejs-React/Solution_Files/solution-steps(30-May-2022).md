...
- sudo pip3 install --upgrade requests    # in order to take any fault messages from Python

- Add the roles_path = /home/ec2-user/ansible/roles to the ansible.cfg.  # to inform cfg files where the roles run..
...
## Part 7 - Prepare playbook with Geerlingguy roles solution.

If we wanna use a prepared roles like geerlingguy, we can search in galaxy hub by using command below;

```
ansible-galaxy search docker --platform EL | grep geerl
```

command will works.. and  you will see such a list;
...
then, select the geerlingguy's docker role's name by the command below;

```
ansible-galaxy install geerlingguy.docker
```

geerlingguy's role will installed in roles directory after the command.

There is an important point for this project after all installation is done before using geerlingguy's role. 
If we wanna to use geerlingguy role's except other solutions(wide or one solution) while creating play-newrole.yml file we must use "shell" module while creating "image" and "container" to take any fault. If we use the other solution files and run the other playbooks before using of geerlingguy role, we must add two plays block seen below to take any fault.
