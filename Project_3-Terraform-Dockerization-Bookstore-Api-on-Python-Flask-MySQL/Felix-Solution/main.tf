...
provider "github" {
  # Configuration options
  token = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"  # type your token here and userdata below..!!
}
...
resource "aws_instance" "tf-dockerize-project" {
  ami             = "ami-0f9fc25dd2506cf6d"
  instance_type   = "t2.micro"
  key_name        = "felix"
  security_groups = ["docker-sec-grp"]
  tags = {
  ...
  user_data = <<-EOF
          ...
          FOLDER="https://$TOKEN@raw.githubusercontent.com/fikretgit/bookstore/main/"
          ...
          docker build -t fikretdocker/bookstoreapi:latest .
          docker-compose up -d
          EOF
...
resource "aws_security_group" "tf-docker-sec-grp-203" {
  name = "docker-sec-grp"
  tags = {
...
output "website" {
  value = "http://${aws_instance.tf-dockerize-project.public_dns}"
